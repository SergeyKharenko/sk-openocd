...
#ifndef OPENOCD_JTAG_DRIVERS_ETHER_DAP_H
#define OPENOCD_JTAG_DRIVERS_ETHER_DAP_H

#include <stdint.h>

struct ether_dap_io_driver;
...
struct ether_dap {
	void *sock;
	const struct ether_dap_io_driver *drv;
...
struct ether_dap_io_driver{
	int (*open)(struct ether_dap *dap, const uint8_t *ip, uint16_t port);
	void (*close)(struct ether_dap *dap);
	int (*read)(struct ether_dap *dap, int transfer_timeout_ms,
			    struct timeval *wait_timeout);
	int (*write)(struct ether_dap *dap, int len, int timeout_ms);
	int (*packet_buffer_alloc)(struct ether_dap *dap, unsigned int pkt_sz);
	void (*packet_buffer_free)(struct ether_dap *dap);
	void (*cancel_all)(struct ether_dap *dap);
};
